...
from experiments.interaction.utils import create_x_y, create_one_hot_x_x_val, create_mean_imputing_x_x_val, make_dirs, \
    all_experiments
...
def worker(model_name, variant, exp_number, category_size, a):
    ...
    results_df = pd.DataFrame(
        columns=['model', 'categories', 'exp', 'a', 'gain', 'permutation_train', 'permutation_test', 'shap_train',
                 'shap_test'])
    results_df.loc[0, ['model', 'categories', 'exp', 'a']] = [F"{model_name}_{variant}", category_size, exp_number, a]
    # fi gain
    if model_name == 'ours':
    ...
    if model_name in ['sklearn', 'xgboost']:
        ...
        fi_shap_train = get_x1_shap_value(reg, train_pool, columns=X_train.columns),
        fi_shap_test = get_x1_shap_value(reg, val_pool, columns=X_test.columns)
    ...
    #     shap.TreeExplainer(reg, feature_perturbation="tree_path_dependent").shap_values(train_pool),
    #     columns=X_train.columns).apply(np.abs)
    # print(abs_shap_values.mean() / abs_shap_values.mean().sum())
    explainer = shap.KernelExplainer(reg.predict, X_train)
    shap_values = explainer.shap_values(X_train, nsamples=100)

    abs_shap_values = pd.DataFrame(shap_values, columns=X_train.columns).append(np.abs)
    print(abs_shap_values.mean() / abs_shap_values.mean().sum())
