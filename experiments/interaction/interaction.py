...
def worker(exp_number, category_size, a, predictor_name, predictor):
    model_type = 'ours'
    ...
    temp_results = pd.DataFrame([[F"{model_type}_{predictor_name}",  category_size, a, fi_gain['x1'], fi_permutation_train['x1'], fi_permutation_test['x1']]])
...
if __name__ == '__main__':
    ...
    for exp_number, category_size, a in tqdm(all_experiments(), total=n_experiments):
        ...
        for predictor_name, predictor in {'Kfold': CartGradientBoostingRegressorKfold,'CartVanilla': CartGradientBoostingRegressor}.items():
