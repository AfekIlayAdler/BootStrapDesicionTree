...
def sklearn_worker(exp_number, category_size, a, predictor_name, _):
    model_type= 'sklearn'
    ...
    temp_results = pd.DataFrame([[F"{model_type}_{predictor_name}", category_size, a, fi_gain['x1'], fi_permutation_train['x1'], fi_permutation_test['x1'], get_x1_shap_value(model, X_train),get_x1_shap_value(model, X_test)]])
...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool(4) as process_pool:
        process_pool.starmap(sklearn_worker, args)